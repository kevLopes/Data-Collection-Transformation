...
def data_collector_piping(project_number, material_type):
    ...
    if project_number == "17033":
        ...
        #Prosperity section
        # Set the search directory and keyword
        search_dir = "../Data Pool/Data Hub Materials/MP17043 PROSPERITY"
        keyword = material_type
        ...
        for file in files:
            try:
                ...
                if df["Project Number"].astype(str).str.contains(str(project_number)).any():
                    ...
                    if extract_columns:
                        ...
                        for project_number, group_df in grouped_df:
                            ...
                            output_filename = os.path.join(output_dir,
                                                           f"{project_number} - Piping Organized_{timestamp}.xlsx")
                            group_df.to_excel(output_filename, index=False)
                    ...
                    logging.error(
                        f"No files were found for the project {project_number} for the {material_type} material")
                    return False
                ...
                if df["Project Number"].astype(str).str.contains(str(project_number)).any():
                    ...
                    if extract_columns:
                        ...
                        for project_number, group_df in grouped_df:
                            ...
                            # Save the group data to a new Excel file with the project number in the filename
                            output_filename = os.path.join(output_dir,
                                                           f"{project_number} - Yard Piping Organized_{timestamp}.xlsx")
                            group_df.to_excel(output_filename, index=False)
                    ...
                    logging.error(
                        f"No files were found for the project {project_number} for the {material_type} material (YARD Scope)")
                    return False
        ...
        for file in files:
            try:
                ...
                if extract_columns:
                    ...
                    # Filter rows where 'Status' is equal to "CONFIRMED" and 'Remarks' does not contain "Yard"
                    extract_df = extract_df[(extract_df['Tag Status'] == "New") | (extract_df['Scope Of Supply'] == "SBM")]
                    ...
                    for project_number, group_df in grouped_df:
                        ...
                        # Save the group data to a new Excel file with the project number in the filename
                        output_filename = os.path.join(output_dir,
                                                       f"{project_number} - Valve Organized_{timestamp}.xlsx")
                        group_df.to_excel(output_filename, index=False)
            ...
            try:
                ...
                if extract_columns:
                    ...
                    extract_df = extract_df[
                        (extract_df['Tag Status'] == "New") | (extract_df['Scope Of Supply'] == "YARD")]
...
def data_collector_bolt_yard(project_number, material_type):
    ...
    if project_number == "17033":
        ...
        # Set the columns to extract
        columns_to_extract = ["Tag Number", "ID", "Project Number", "Product Code", "Commodity Code",
                              "Service Description", "Pipe Base Material", "Material", "Total NET weight",
                              "SBM scope", "Qty confirmed in design", "Total QTY to commit", "Quantity UOM",
                              "SIZE", "Quantity to complete", "Total Qty to complete weight"]
            ...
            try:
                ...
                if df["Project Number"].astype(str).str.contains(str(project_number)).any():
                    ...
                    if extract_columns:
                        ...
                        # Filter rows where 'SBM scope' is equal to True
                        extract_df = extract_df[(extract_df['Pipe Base Material'] != "NICKEL ALLOY") & (extract_df['Pipe Base Material'].notnull())]
                        ...
                        for project_number, group_df in grouped_df:
                            ...
                            # Save the group data to a new Excel file with the project number in the filename
                            output_filename = os.path.join(output_dir,
                                                           f"{project_number} - Bolt Organized_{timestamp}.xlsx")
                            group_df.to_excel(output_filename, index=False)
...
def data_collector_structure(project_number, material_type):
    ...
    if project_number == "17033":
        ...
        columns_to_extract = ["Tag Number", "Project", "Product Code", "Commodity Code",
                              "Service Description", "Thickness", "Material", "Wastage Quantity",
                              "Required Qty", "Unit Weight", "Total QTY to commit", "Quantity UOM",
                              "Unit Weight UOM", "Total NET weight", "Quantity Including Wastage", "Total Gross Weight",
                              "Total Gross Weight UOM"]
                ...
                if df["Project"].astype(str).str.contains(str(project_number)).any():
                    ...
                    if extract_columns:
                        ...
                        for project_number, group_df in grouped_df:
                            ...
                            # Save the group data to a new Excel file with the project number in the filename
                            output_filename = os.path.join(output_dir,
                                                           f"{project_number} - Structure Organized_{timestamp}.xlsx")
                            group_df.to_excel(output_filename, index=False)
...
def data_collector_specialpip(project_number, material_type):
    ...
    if project_number == "17033":
        ...
        # Set the columns to extract
        columns_to_extract = ["TAG Number", "Tag Status", "Type", "Size (Inch)", "Description", "Dry Weight [kg]", "Dry Weight Source", "Service", "Remarks", "PO Number",
                              "Project Number", "Scope Of Supply", "Product Code", "Min Design Temperature [degC]", "Max Design Temperature [degC]", ]
        ...
        for file in files:
            try:
                ...
                if extract_columns:
                    ...
                    # Save the data to a new Excel file with the project number in the filename
                    output_filename = os.path.join(output_dir,
                                                   f"SO{project_number} - SPC Piping Organized_{timestamp}.xlsx")
                    extract_df.to_excel(output_filename, index=False)
