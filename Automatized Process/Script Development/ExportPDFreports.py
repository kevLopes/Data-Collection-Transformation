...
def generate_pdf_piping_sbm_scope(data, project_number):
    ...
    pdf.cell(0, 10, f"Date of Analysis: {date.today().strftime('%d-%b-%Y')}", ln=True, align="L")
    ...
    for material_type, group in grouped_data:
        ...
        filtered_data = group[group['Quantity UOM'].isin(['METER', 'PCS'])]
        pdf.cell(0, 10, f"Total QTY to commit: {filtered_data['Total QTY to commit'].sum()}", ln=True, align="L")
    ...
    # Save the PDF report
    pdf_path = os.path.join(directory_path, f"{project_number}_MTOPipingReportSBM.pdf")
    pdf.output(pdf_path)


def generate_report(data, project_number, file_name):
    # Convert Total NET weight to TON
    data['Total NET weight'] = data['Total NET weight'] / 1000
